...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const { data: post } = await locals.supabase.from('posts').select('*').eq('showcase_id', showcase.id).eq('slug', params.post).eq('status', 'published').maybeSingle();
  if (!post) error(404, 'Publication introuvable');
  return { showcase, post };
};
